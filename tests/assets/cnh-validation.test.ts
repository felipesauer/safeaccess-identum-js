...
describe(CNHValidation.name, () => {
    it('validates CNH numbers as true (unmasked and with noise)', () => {
        expect(new CNHValidation('22522791508').isValid()).toBe(true);
        expect(new CNHValidation('12345678900').isValid()).toBe(true);
        expect(new CNHValidation('  225227915-08 ').isValid()).toBe(true);
    });

    it('rejects wrong check digits (DV mismatch)', () => {
        expect(new CNHValidation('22522791509').isValid()).toBe(false);
        expect(new CNHValidation('92079525000').isValid()).toBe(false);
    });

    it('rejects wrong lengths and repeated sequences', () => {
        expect(new CNHValidation('2252279150').isValid()).toBe(false);
        expect(new CNHValidation('225227915080').isValid()).toBe(false);
        expect(new CNHValidation('00000000000').isValid()).toBe(false);
        expect(new CNHValidation('11111111111').isValid()).toBe(false);
    });

    it('whitelist short-circuits to valid even if DVs are wrong', () => {
        const v = new CNHValidation('99999999999').allowList(['99999999999']);
        expect(v.isValid()).toBe(true);
        expect(v.validateOrFail()).toBeUndefined();
    });

    it('blacklist short-circuits to invalid even if domain is valid', () => {
        const v = new CNHValidation('22522791508').denyList(['22522791508']);
        expect(v.isValid()).toBe(false);
        expect(() => v.validateOrFail()).toThrow(ValidationException);
        expect(() => v.validateOrFail()).toThrow('cnh: denied');
    });
    ...
    it('hits the dv2 adjustment branch (dv1=10→0 and dv2-2<0 → +9)', () => {
        expect(new CNHValidation('10005500000').isValid()).toBe(true);
    });

    it('hits dv2 -= 2 branch (firstIsTenPlus=true and dv2-2 >= 0)', () => {
        expect(new CNHValidation('10000003600').isValid()).toBe(true);
    });

    it('hits dv2 > 9 clamp in normal path (sum2%11 == 10)', () => {
        expect(new CNHValidation('10000001550').isValid()).toBe(true);
    });

    it('validates CNH where dv1 == 9 (boundary, not clamped)', () => {
        expect(new CNHValidation('00000000994').isValid()).toBe(true);
    });

    it('validates CNH where dv2 == 9 without firstIsTenPlus (boundary, not clamped)', () => {
        expect(new CNHValidation('90000000049').isValid()).toBe(true);
    });

    it('rejects CNH with wrong dv1 but correct dv2', () => {
        expect(new CNHValidation('22522791518').isValid()).toBe(false);
    });
