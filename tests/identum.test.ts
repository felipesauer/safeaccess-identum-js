...
describe(Identum.name, () => {
    ...
    it('returned validator supports whitelist()', () => {
        expect(Identum.cpf('323.543.123-43').allowList(['323.543.123-43']).isValid()).toBe(true);
    });

    it('returned validator supports blacklist()', () => {
        expect(Identum.cpf('864.600.120-24').denyList(['864.600.120-24']).isValid()).toBe(false);
    });

    it('returned validator supports validateOrFail()', () => {
        expect(Identum.cpf('864.600.120-24').validateOrFail()).toBeUndefined();
